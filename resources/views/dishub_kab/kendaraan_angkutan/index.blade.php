...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.dishub-kab.kendaraan-angkutan.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.dishub-kab.kendaraan-angkutan.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
            ...
            swal.fire({
                ...
                if (result) {
                    const postDataRest = await CallAPI(
                        'DELETE',
                        `{{ route('api.dishub-kab.kendaraan-angkutan.destroy') }}`, 
                        {
                            "id": id
                        }
                    ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const data = {}
            let method = "POST"
            let url = `{{ route('api.dishub-kab.kendaraan-angkutan.create') }}`
            if (isActionForm == "update") {
                url = `{{ route('api.dishub-kab.kendaraan-angkutan.update') }}`
                data["id"] = $("#inputId").val()
                method      = "PUT"
            }
    ...
    async function initPageLoad() {
        await Promise.all([
            ...
            // await showSelectList('#input_provinsi_id', `{{ route('api.dishub-kab.master.provinsi.list') }}`),
            // await showSelectList('#input_kota_id', `{{ route('api.dishub-kab.master.kota.list') }}`),

            await showSelectList('#input_perusahaan_id', `{{ route('api.dishub-kab.master.perusahaan.list') }}`),
            await showSelectList('#input_jenis_angkutan_id', `{{ route('api.dishub-kab.master.jenis-angkutan.list') }}`),

            await showSelectList('#input_trayek_id', `{{ route('api.dishub-kab.master.trayek.list') }}`),
            await showSelectList('#input_rute_trayek_id', `{{ route('api.dishub-kab.master.rute-trayek.list') }}`),
            await showSelectList('#input_jenis_layanan_id', `{{ route('api.dishub-kab.master.jenis-layanan.list') }}`),
            await showSelectList('#input_kendaraan_id', `{{ route('api.dishub-kab.master.kendaraan.list') }}`),
