...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.dishub-kab.master.perusahaan.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.dishub-kab.master.perusahaan.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
            ...
            swal.fire({
                ...
                if (result) {
                    const postDataRest = await CallAPI(
                        'DELETE',
                        `{{ route('api.dishub-kab.master.perusahaan.destroy') }}`, 
                        {
                            "id": id
                        }
                    ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const data = {}
            let method = "POST"
            let url = `{{ route('api.dishub-kab.master.perusahaan.create') }}`
            if (isActionForm == "update") {
                data["id"] = $("#inputId").val()
                url = `{{ route('api.dishub-kab.master.perusahaan.update') }}`
                method      = "PUT"
            }
