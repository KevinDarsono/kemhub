...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.administrator.master.kendaraan.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.administrator.master.kendaraan.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
            ...
            swal.fire({
                ...
                if (result) {
                    const postDataRest = await CallAPI(
                        'DELETE',
                        `{{ route('api.administrator.master.kendaraan.destroy') }}`, 
                        {
                            "id": id
                        }
                    ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const data = {}
            let method = "POST"
            let url = `{{ route('api.administrator.master.kendaraan.create') }}`
            if (isActionForm == "update") {
                url = `{{ route('api.administrator.master.kendaraan.update') }}`
                data["id"] = $("#inputId").val()
                method      = "PUT"
            }
