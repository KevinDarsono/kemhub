...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.administrator.master.trayek.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.administrator.master.trayek.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
            ...
            swal.fire({
                ...
                if (result) {
                    const postDataRest = await CallAPI(
                        'DELETE',
                        `{{ route('api.administrator.master.trayek.destroy') }}`, 
                        {
                            "id": id
                        }
                    ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const postDataRest = await CallAPI(
                method,
                `{{ route('api.administrator.master.trayek.update') }}`, 
                data
            ).then(function(response) {
                return response
            }).catch(function(error) {
