...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.administrator.master.rute-trayek.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    function addData() {
        $(document).on("click", "#add-data", function() {
            ...
            $("#modalData").appendTo("body").modal("show")
        })
    }
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.administrator.master.rute-trayek.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
            ...
            if (getDataRest.status == 200) {
                ...
                let domHtml = `
                    <tr>
                        ...
                        <td>: ${data.trayek.nama?data.trayek.nama:'-'}</td>
                    </tr>
                    <tr>
                        <th>Nama Rute</th>
            ...
            swal.fire({
                ...
                if (result) {
                    const postDataRest = await CallAPI(
                        'DELETE',
                        `{{ route('api.administrator.master.rute-trayek.destroy') }}`, 
                        {
                            "id": id
                        }
                    ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const postDataRest = await CallAPI(
                method,
                `{{ asset('api.administrator.master.rute-trayek.${isActionForm}') }}`, 
                data
            ).then(function(response) {
                return response
            }).catch(function(error) {
            ...
            if (postDataRest.status == 200) {
                ...
                notificationAlert('success', 'Pemberitahuan', postDataRest.data.message)
                $(this).trigger("reset")
                $("#modalData").appendTo("body").modal("hide")
            }
        })
    }
