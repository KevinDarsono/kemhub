    ...
    <script>
        ...
        async function editData() {
            $(document).on("click", ".edit-data", async function() {
                ...
                if (getDataRest.data.status_code == 200) {
                    loadingPage(false);
                    $("form").find("select").val("").prop("selected", false).trigger("change");
                    $(".modal-title").html(modalTitle);
                    $("#modalData").modal("show");
                    $('#tableEdit').show();
                    ...
                    data.forEach((editData, index) => {
                        ...
                        $('#inputId').val(editData.id); // Corrected here
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <button class="btn btn-danger btn-sm delete-data" type="button" title="Hapus" data-id="${editData.id}">
        ...
        async function submitData() {
            $(document).on("submit", "#form-data", async function(e) {
                ...
                if (isActionForm == "update") {
                    ...
                    data["id"] = $("#inputId").val(); // Ensure the ID is correctly set
                    method = "PUT";
