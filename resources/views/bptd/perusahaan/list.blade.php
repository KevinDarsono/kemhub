        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        @if (Auth::user()->code_role == App\Constants\Roles::ADMINISTRATOR)
                        <div class="col-md-6">
...
<script>
    ...
    async function getListData(limit = 10, page = 1, ascending = 0, search = '') {
        ...
        const getDataRest = await CallAPI(
            'GET',
            `{{ route('api.bptd.master.perusahaan.list') }}`, 
            {
                page: page,
                limit: limit,
                ascending: ascending,
    ...
    async function detailData() {
        $(document).on("click", ".detail-data", async function() {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ route('api.bptd.master.perusahaan.find') }}`, 
                {
                    id: id
                }
            ).then(function(response) {
    ...
    async function submitData() {
        $(document).on("submit", "#form-data", async function(e) {
            ...
            const data = {}
            let method = "POST"
            let url = `{{ route('api.bptd.master.perusahaan.list') }}`
            if (isActionForm == "update") {
                data["id"] = $("#inputId").val()
                url = `{{ route('api.bptd.master.perusahaan.list') }}`
                method      = "PUT"
            }
